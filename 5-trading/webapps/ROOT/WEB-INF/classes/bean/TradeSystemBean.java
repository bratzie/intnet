...
public class TradeSystemBean {
	...
	private String security = ""; //This variable should be updated through the controller each request.
	...
	public ArrayList<ClosedOrderBean> getTrades(){
		...
		for(ClosedOrderBean order : closedOrders){
			if(order.getName().equals(security)){
	...
	public void handleViewTrades(String parameter) {
		security = parameter.toUpperCase().trim();
	}
